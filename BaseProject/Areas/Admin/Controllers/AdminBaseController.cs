...
namespace AJAD.Areas.Admin.Controllers
{
    public class AdminBaseController : Controller
    {
        ...
        {
            ViewBag.BaseUrl = "http://" + Request.Url.Host + (Request.Url.Port == 80 ? "" : ":" + Request.Url.Port) + "/admin";
                ...
                if (LOGGEDIN_USER == null)
                {
                    ...
                    catch (Exception exc)
                    {
                        if (auth_cookie != null)
                        {
                            ...
                            filter_context.Result = RedirectToAction("Index", "Home", new { returnUrl = Request.Url.AbsolutePath, Areas = "Admin" });
            ...
            {
                if (!Request.IsAjaxRequest()) filter_context.Result = RedirectToAction("Index", "Home", new { returnUrl = Server.UrlEncode(Request.RawUrl), Areas = "Admin" });
        ...
        public virtual ActionResult SignOut()
        {
            ...
            return Redirect(Url.Action("Index", "Home", new { Areas = "Admin" }));
        }
